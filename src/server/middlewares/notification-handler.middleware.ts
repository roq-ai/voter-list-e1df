...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'ElectionCreated.create': {
    roles: ['admin-user', 'election-officer', 'data-entry-operator', 'voter'],
    key: 'electionId',
    tenantPath: [],
    userPath: [],
  },
  'ElectionUpdated.update': {
    roles: ['admin-user', 'election-officer', 'data-entry-operator', 'voter'],
    key: 'electionId',
    tenantPath: [],
    userPath: [],
  },
  'ElectionDeleted.delete': {
    roles: ['admin-user', 'election-officer', 'data-entry-operator', 'voter'],
    key: 'electionId',
    tenantPath: [],
    userPath: [],
...
const tenantRoles: string[] = ['admin-user', 'election-officer', 'data-entry-operator', 'voter'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.party.findFirst({
